...
@Component({
  selector: 'app-quejas-rechazadas',
  templateUrl: './quejas-rechazadas.page.html',
  styleUrls: ['./quejas-rechazadas.page.scss'],
})
export class QuejasRechazadasPage implements OnInit {
      ...
      rp.forEach(elementRP => {
        this.restauranteSvc.listar().subscribe(res =>{
          ...
          res.forEach(elementR => {
            if(this.usuarioLog === elementRP.uidResta && elementR.userUID === elementRP.uidResta && elementRP.estado === 'falso'){
