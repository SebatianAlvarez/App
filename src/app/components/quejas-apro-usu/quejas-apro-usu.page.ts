...
@Component({
  selector: 'app-quejas-apro-usu',
  templateUrl: './quejas-apro-usu.page.html',
  styleUrls: ['./quejas-apro-usu.page.scss'],
})
export class QuejasAproUsuPage implements OnInit {
      ...
      rp.forEach(elementRP => {
        this.restauranteSvc.listar().subscribe(res =>{
          ...
          res.forEach(elementR => {
            if(this.usuarioLog === elementRP.uidUsu && elementR.userUID === elementRP.uidResta && elementRP.estado === 'verdadero'){
